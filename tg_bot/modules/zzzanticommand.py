...
def rem_cmds(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not args:
        ...
        if del_pref:
            update.effective_message.reply_text("I should be deleting blue text messages now.")
        else:
            update.effective_message.reply_text("I'm currently not deleting blue text messages!")
        return ""

    if args[0].lower() in ("on", "yes"):
        sql.set_cmd_joined(str(chat.id), True)
        update.effective_message.reply_text("I'll try to delete blue text messages!")
        return "<b>{}:</b>" \
               "\n#ANTI_COMMAND" \
               "\n<b>Admin:</b> {}" \
        ...
        update.effective_message.reply_text("I won't delete blue text  messages.")
        return "<b>{}:</b>" \
               "\n#ANTI_COMMAND" \
               "\n<b>Admin:</b> {}" \
...
- /rmcmd <on/off>: when someone tries to send a Blue Text message, I will try to delete that!
"""
